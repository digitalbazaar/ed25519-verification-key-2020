...
export class Ed25519VerificationKey2020 extends LDKeyPair {
  ...
  constructor(options = {}) {
    ...
    if(!this.publicKeyMultibase ||
       !_isValidKey(this.publicKeyMultibase, MULTICODEC_ED25519_PUB_HEADER)) {
      throw new Error('`publicKeyMultibase` has invalid byte header: ' +
      `"${this.publicKeyMultibase}".`);
    }

    if(this.privateKeyMultibase &&
       !_isValidKey(this.privateKeyMultibase, MULTICODEC_ED25519_PRIV_HEADER)) {
      throw new Error('`privateKeyMultibase` has invalid byte header: ' +
  ...
  static fromEd25519VerificationKey2018({keyPair} = {}) {
    // 'z' is base58btc alphabet in multibase
    const publicKeyMultibase = MULTIBASE_BASE58BTC_HEADER +
      base58btc.encode(new Uint8Array([
    ...
    if(keyPair.privateKeyBase58) {
      // 'z' is base58btc alphabet in multibase
      const privateKeyMultibase = MULTIBASE_BASE58BTC_HEADER +
        base58btc.encode(new Uint8Array([
          ...MULTICODEC_ED25519_PRIV_HEADER,
          ...base58btc.decode(keyPair.privateKeyBase58)
        ]));
      keyPair2020.privateKeyMultibase = privateKeyMultibase;
    }
  ...
  get _privateKeyBuffer() {
    ...
    const privateKeyBytes =
      privateKeyMulticodec.slice(MULTICODEC_ED25519_PUB_HEADER.length);
   ...
   */
  fingerprint() {
    const publicKeyMulticodec = new Uint8Array([
      ...MULTICODEC_ED25519_PUB_HEADER,
      ...this._publicKeyBuffer
    ]);

    return MULTIBASE_BASE58BTC_HEADER + base58btc.encode(publicKeyMulticodec);
  }
...
function _isValidKey(multibaseKey, expectedHeader) {
  ...
  return !expectedHeader.some((val, i) => {
    return keyBytes[i] !== val;
  });
}
