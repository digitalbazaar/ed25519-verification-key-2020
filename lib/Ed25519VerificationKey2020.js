...
export class Ed25519VerificationKey2020 extends LDKeyPair {
  ...
  constructor(options = {}) {
    super(options);
    this.type = SUITE_ID;
    const publicKeyMultibase = options.publicKeyMultibase;
    const privateKeyMultibase = options.privateKeyMultibase;
    ...
    if(!publicKeyMultibase || !_isValidKeyHeader(
      ...
      throw new Error(
        '`publicKeyMultibase` has invalid byte header: ' +
        `"${publicKeyMultibase}".`);
    }
    ...
    if(privateKeyMultibase && !_isValidKeyHeader(
      ...
      throw new Error(
        '`privateKeyMultibase` has invalid byte header: ' +
        `"${privateKeyMultibase.slice(0, 4)}".`);
    }
