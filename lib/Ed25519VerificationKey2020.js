...
export class Ed25519VerificationKey2020 extends LDKeyPair {
  ...
  constructor(options = {}) {
    ...
    if(!this.publicKeyMultibase || !_isValidKeyHeader(
      this.publicKeyMultibase, MULTICODEC_ED25519_PUB_HEADER)) {
      throw new Error('`publicKeyMultibase` has invalid byte header: ' +
      `"${this.publicKeyMultibase}".`);
    }

    if(this.privateKeyMultibase && !_isValidKeyHeader(
      this.privateKeyMultibase, MULTICODEC_ED25519_PRIV_HEADER)) {
