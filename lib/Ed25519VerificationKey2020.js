...
export class Ed25519VerificationKey2020 extends LDVerifierKeyPair {
  ...
  static async generate({seed, secureRandom, ...keyPairOptions} = {}) {
    ...
    if(seed) {
      keyObject = await crypto.generateKeyPairFromSeed(seed);
    } else {
      keyObject = await crypto.generateKeyPair({
  ...
  signer() {
    ...
    return {
      async sign({data}) {
        return crypto.sign(privateKeyBuffer, data);
      },
    };
  }
  ...
  verifier() {
    ...
    return {
      async verify({data, signature}) {
        return crypto.verify(publicKeyBuffer, data, signature);
      },
    };
  }
}
