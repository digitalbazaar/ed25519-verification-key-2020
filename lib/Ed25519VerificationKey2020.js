...
export class Ed25519VerificationKey2020 extends LDVerifierKeyPair {
   ...
   *
   * @param {object} [options={}] - Options hashmap.
   * @param {Uint8Array|Buffer} [options.seed] - A 32-byte array seed for a
   *   deterministic key.
   * @param {Function} [options.secureRandom] - Optional sync function that
   *   returns a Uint8Array of random bytes. Used as override, for use in
   *   environments other than Node.js or browser. Defaults to
   *   `@stablelib/random.randomBytes()`.
   *
   * @returns {Promise<Ed25519VerificationKey2020>} Resolves with generated
   *   public/private key pair.
   */
  static async generate({seed, secureRandom, ...keyPairOptions} = {}) {
    let keyObject;
    if(!secureRandom) {
      secureRandom = () => randomBytes(32);
    }
    if(seed) {
      keyObject = await key.generateKeyPairFromSeed(seed);
    } else {
      keyObject = await key.generateKeyPair({
  ...
  signer() {
    ...
    return {
      async sign({data}) {
        return key.sign(privateKeyBuffer, data);
      },
    };
  }
  ...
  verifier() {
    ...
    return {
      async verify({data, signature}) {
        return key.verify(publicKeyBuffer, data, signature);
      },
    };
  }
}
