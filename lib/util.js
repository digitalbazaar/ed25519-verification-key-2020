...
const DER_PUBLIC_KEY_PREFIX = Buffer.from(
  [0x30, 0x2a, 0x30, 0x05, 0x06, 0x03, 0x2b, 0x65, 0x70, 0x03, 0x21], 'hex');
...
export function publicKeyDerEncode({publicKeyBytes}) {
  ...
  const keyBuffer = Buffer.concat([zeroBuffer, publicKeyBytes]);
  return Buffer.concat([DER_PUBLIC_KEY_PREFIX, keyBuffer]);
}
